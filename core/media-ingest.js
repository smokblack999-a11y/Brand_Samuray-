...
function mediaFromMessage(message) {
  ...
  if (message?.video?.file_id) {
    return { type: "video", fileId: message.video.file_id, width: message.video.width, height: message.video.height, duration: message.video.duration };
  }
  if (message?.document?.file_id) {
    return { type: "document", fileId: message.document.file_id, fileName: message.document.file_name || null, mimeType: message.document.mime_type || null };
  }
  return null;
}
...
async function downloadTelegramFile(fileId, originalName = "media.bin") {
  ...
  const output = path.join(MEDIA_DIR, crypto.randomUUID() + (ext || ".bin"));
