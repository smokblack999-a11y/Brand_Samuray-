...
function createRecoveryRouter({ requireRecoveryAuth }) {
  ...
  router.post("/github", requireRecoveryAuth, (req, res) => {
    try {
      ...
      if(!failures.includes(job.conclusion)) return res.status(202).json({ok:true,accepted:false,reason:"unsupported_conclusion",job});
      ...
      if(existing){
        ...
        return res.status(200).json({ok:true,accepted:true,created:false,reused:true,job:updated});
      }
      const result=enqueue({...job,fingerprint:fp,diagnosis,failureLogs:logs.slice(-12000)});
      const critic=decide({attempts:result.job.attempts,evidence:diagnosis.evidence,patch:{changedFiles:0,changedLines:0}});
      const saved=result.created?update(result.job.id,{diagnosis,critic,status:"queued"}):result.job;
