...
function errorBody(code, message, requestId, extra = {}) {
  return { ok: false, error: { code, message, requestId }, ...extra };
}
...
function requestId(req,res,next) {
  const id=crypto.randomUUID(); req.requestId=id; res.setHeader("X-Request-Id",id); next();
}
app.use(requestId);
...
app.get("/ready",(req,res)=>{
  try{
    const current=stats(), audit=verifyChain(), required=process.env.NODE_ENV==="production"?requiredEnv():[];
...
app.post("/api/recovery/state/transition",requireRecoveryAuth,(req,res)=>{
  try{
    const current=req.body?.job,nextState=req.body?.nextState,evidence={proof:req.body?.proof||{}};
    if(!current?.id||!current?.repository||!current?.headSha||!current?.diffHash)return res.status(400).json(errorBody("RECOVERY_JOB_IDENTITY_REQUIRED","job id, repository, headSha and diffHash are required",req.requestId));
    const result=transition(current,nextState,evidence);
    if(!result.ok)return res.status(409).json({...result,requestId:req.requestId});
    return res.status(200).json({...result,trusted:result.job.state===STATES.VERIFIED,requestId:req.requestId});
  }catch(error){return res.status(500).json(errorBody("RECOVERY_STATE_TRANSITION_FAILED",error.message,req.requestId));}
});
async function analyze(message,business){
  const text=String(message||"").trim();
  if(!text){const e=new Error("message обязателен");e.code="INVALID_MESSAGE";throw e;}
  if(text.length>MAX_MESSAGE_CHARS){const e=new Error(`message слишком длинный (максимум ${MAX_MESSAGE_CHARS} символов)`);e.code="MESSAGE_TOO_LONG";throw e;}
...
app.post("/api/lead/analyze",requireApiKey,leadRateLimit,async(req,res)=>{
  try{const {message,business}=req.body||{},result=await analyze(message,business),saved=saveLead({source:"api",message:String(message).trim(),...result.lead,reply:result.reply});res.json({ok:true,...result,saved,requestId:req.requestId});}
...
app.post("/api/telegram/webhook",requireWebhookSecret,async(req,res)=>{
  ...
  try{
    ...
    if(update.business_connection){console.log(JSON.stringify({event:"business_connection",id:update.business_connection.id,requestId:req.requestId}));return;}
...
server.requestTimeout=REQUEST_TIMEOUT_MS;server.headersTimeout=REQUEST_TIMEOUT_MS+5000;
