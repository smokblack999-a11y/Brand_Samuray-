...
function eventKeyFor(run) {
  return `github:workflow_run:${run.repository || "unknown"}:${run.runId || run.runNumber || "unknown"}`;
}
function enqueue(input = {}) {
  if (!input.repository || (!input.runId && !input.runNumber)) throw new Error("repository and workflow_run.id are required");
  return withLock(() => {
    const rows = read();
    const eventKey = input.eventKey || eventKeyFor(input);
    const existing = rows.find(x => x.eventKey === eventKey);
    if (existing) return { created: false, job: existing };
    const id = `recovery-${crypto.createHash("sha256").update(eventKey).digest("hex").slice(0, 24)}`;
    const now = new Date().toISOString();
    const job = {
      id, eventKey, createdAt: now, updatedAt: now,
      state: "queued", status: "queued", attempts: 0,
      maxAttempts: Math.max(1, Number(input.maxAttempts || process.env.RECOVERY_MAX_ATTEMPTS || 3)),
      repository: String(input.repository),
      workflow: input.workflow || null,
      runId: String(input.runId || input.runNumber),
      runNumber: input.runNumber || null,
      branch: input.branch || null,
      headSha: input.sha || null,
      failureLogs: String(input.failureLogs || "").slice(-12000),
      diagnosis: input.diagnosis || null,
      patch: null,
      proofReceipt: null
    };
    rows.push(job); write(rows);
