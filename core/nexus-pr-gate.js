...
function assertBranch(value, name) {
  ...
  if (!branch || branch.length > 255 || /[\u0000\n\r]/.test(branch)) {
    throw new Error(name + " is required and must be a valid branch name");
  }
  return branch;
}
...
function validateShipReceipt(receipt, { headSha } = {}) {
  ...
  if (receipt.decision !== "SHIP" || receipt.humanRequired !== false) {
    throw new Error("only a verified SHIP receipt can create a draft PR");
  }
...
async function createVerifiedDraftPr({ githubClient, repository, head, base = "main", title = "NEXUS: verified repair", summary = "", receipt, evidenceUrl = "" } = {}) {
