/**
 * X10THINC X27 Persistent Repair Worker contract.
 *
 * Pure deterministic state machine. Persistence is supplied by the caller
 * (database/file/queue adapter); this module never performs network I/O,
 * filesystem writes, git push, merge, or deployment.
 */
...
function transition(job, next, meta = {}) {
  ...
  const allowed = {
    ...
    [STATES.TESTING]: [STATES.PROVEN, STATES.PATCHING, STATES.STOPPED],
  ...
  const record = { state: next, attempt: attempts, ...meta };
  return Object.freeze({ ...job, state: next, attempts, history: [...job.history, record] });
}
