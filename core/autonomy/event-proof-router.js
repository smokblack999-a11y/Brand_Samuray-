...
/**
 * X26 workflow_run router.
 * Normalizes GitHub Actions failures into deterministic repair-job input.
 * No patching, git push, merge, or deployment occurs here.
 */

const { diagnose } = require("./proof-repair-engine");

const CONCLUSIONS = new Set(["failure", "timed_out", "startup_failure", "cancelled"]);
...
function routeWorkflowRun(event, { existingJobKeys = new Set() } = {}) {
  ...
  if (run.conclusion === "success") return Object.freeze({ action: "ignore_success", jobKey, run });
  if (!CONCLUSIONS.has(run.conclusion)) return Object.freeze({ action: "ignore_unsupported", jobKey, run });
  if (existingJobKeys.has(jobKey)) return Object.freeze({ action: "dedupe", jobKey, run });
